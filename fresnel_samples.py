# -*- coding: utf-8 -*-
"""
Created on Fri Feb 24 14:12:45 2012
...
def sample2():
    ...
    material_nk_data = array([[200, 2.1+0.1j],
                              ...
                              [500, 2.2+0.4j],
                              [750, 2.2+0.5j]])
    material_n_fn = interp1d(material_nk_data[:,0].real, 
                             material_nk_data[:,1].real, kind='quadratic')
    material_k_fn = interp1d(material_nk_data[:,0].real,
                             material_nk_data[:,1].imag, kind='quadratic')
    d_list = [inf,300,inf] #in nm
    lambda_list = linspace(200,750,400)
    T_list = []
    for lambda_vac in lambda_list:
        n_list = [1, material_n_fn(lambda_vac)
                     + 1j*material_k_fn(lambda_vac), 1]
